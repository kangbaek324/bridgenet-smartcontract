...
describe("Bridge Contract Main Test", () => {
    ...
    it("should request cancel", async () => {
        ...
        await expect(bridge
            .connect(owner)
            .cancelRequest(1n)
        ).to.be.revertedWith("only the requester can cancel");
    ...
    it("should request decide", async () => {
        ...
        await expect(bridge
            .connect(owner)
            .setRequest(99n, RequestStatus.approved)
        ).to.be.revertedWithCustomError(bridge, "IncorrectRequestId");

        // 취소된 요청 처리 시도 할때
        await expect(bridge
            .connect(owner)
            .setRequest(1n, RequestStatus.approved)
        ...
        expect((await bridge.requestList(2n)).status).to.equal(RequestStatus.approved);
        expect((await bridge.requestList(2n)).statusDecidedBy).to.equal(await owner.getAddress());
